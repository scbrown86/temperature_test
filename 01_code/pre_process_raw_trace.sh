...
ncremap -g /home/dafcluster4/Documents/GitHub/temperature_test/02_data/00_maps/sahul_coarse.nc -G latlon=15,15#snwe=-48.75,7.5,105.0,161.25 #3.75 grid
...
for file in ./raw/monthly/others/*; do
    ...
    if [[ "$var" == "T" || "$var" == "U" || "$var" == "V" || "$var" == "Z3" ]]; then
        cdo -setreftime,1989-01-16,,1month -settaxis,1989-01-16,,1month -setcalendar,365_day -seltimestep,5269/5280 -sellevidx,20,26 "$infile" "$outname"
    elif [[ "$var" == "RELHUM" ]]; then
        cdo --reduce_dim -setreftime,1989-01-16,,1month -settaxis,1989-01-16,,1month -setcalendar,365_day -seltimestep,5269/5280 -sellevidx,26 "$infile" "$outname"
    else
        cdo -setreftime,1989-01-16,,1month -settaxis,1989-01-16,,1month -setcalendar,365_day -seltimestep,5269/5280 "$infile" "$outname"
    fi
